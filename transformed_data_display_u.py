...
def fetch_transformed_data(selected_dates):
    try:
        ...
        agg_data = df.groupby('EDD_reviewer').agg(
            other_red_flags=('edd_escalation_tasks', lambda x: (x == 'other_red_flags').sum()),
            adverse_media=('edd_escalation_tasks', lambda x: (x == 'adverse_media').sum()),
            high_risk=('edd_escalation_tasks', lambda x: (x == 'high_risk').sum()),
            pep_association=('edd_escalation_tasks', lambda x: (x == 'pep_association').sum()),
            country_risk=('edd_escalation_tasks', lambda x: (x == 'country_risk').sum()),
            complex_ownership=('edd_escalation_tasks', lambda x: (x == 'complex_ownership').sum()),
            young_company=('edd_escalation_tasks', lambda x: (x == 'young_company').sum()),
            medium_risk=('edd_escalation_tasks', lambda x: (x == 'medium_risk').sum()),
            feedback_review=('edd_escalation_tasks', lambda x: (x == 'feedback_review').sum()),
            aml_review=('edd_escalation_tasks', lambda x: (x == 'aml_review').sum()),
            audit_review=('edd_escalation_tasks', lambda x: (x == 'audit_review').sum())
        ).reset_index()
        ...
        agg_data['Total SLA period'] = (
            ...
            agg_data['aml_review'] * weights['aml_review'] +
            agg_data['audit_review'] * weights['audit_review'] +
            agg_data['feedback_review'] * weights['feedback_review']
        )
...
def show():
    ...
    if show_date_picker:
        ...
        with st.container():
            ...
            with col3:
                ...
                <div class="kpi-card" style="background-color: #F9D5E5; color: #333; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); padding: 20px; border-radius: 10px; width: 100%; height: 150px;">
                    <h3><span class="kpi-icon">🔻</span> Lowest Reviewer</h3>  <!-- Down Arrow Icon -->
        ...
        On = st.toggle("Show Trendlines")
        if On:
            st.subheader("Trendlines for EDD and Escalation Tasks")

            # Melting the dataframe for escalation and EDD tasks
            escalation_df = filtered_df.melt(
                id_vars=['EDD_reviewer'],
                value_vars=[
                    'other_red_flags', 'adverse_media', 'high_risk', 'pep_association',
                    'audit_review', 'aml_review', 'feedback_review',
                    'country_risk', 'complex_ownership', 'young_company', 'medium_risk'
                ],
                var_name='Task Type', value_name='Count'
            )
            escalation_df = escalation_df[escalation_df['Count'] > 0]

            # Create a bar chart for task counts
            bar_chart = alt.Chart(escalation_df).mark_bar().encode(
                x=alt.X('sum(Count):Q', title='Total Count'),
                y=alt.Y('EDD_reviewer:N', title='EDD Reviewer'),
                color='Task Type:N',
                tooltip=['EDD_reviewer', 'Task Type', 'sum(Count)']
            ).properties(
                title="Escalation and EDD Task Counts per Reviewer",
                width=600,
                height=400
            )

            # Create a line chart for trendlines
            line_chart = alt.Chart(escalation_df).mark_line(point=alt.OverlayMarkDef(color='red')).encode(
                x=alt.X('sum(Count):Q', title='Total Count'),
                y=alt.Y('EDD_reviewer:N', title='EDD Reviewer'),
                color='Task Type:N',
                tooltip=['EDD_reviewer', 'Task Type', 'sum(Count)']
            ).properties(
                title="Trendline for Total Task Counts per Reviewer",
                width=600,
                height=400
            )

            # Combine the charts using alt.layer (overlapping) or alt.vconcat (stacked)
            combined_chart = alt.layer(bar_chart, line_chart).resolve_scale(
                y='independent'
            ).properties(
                title="Combined Task Counts and Trendlines"
            )
